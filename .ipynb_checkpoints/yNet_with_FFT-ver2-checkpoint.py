...
def apply_threshold(tensor, threshold):
    ...
    binary_tensor = torch.sigmoid((tensor - threshold) * 100)
...
class FYNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x = apply_threshold(x, self.threshold)
...
def main():
    ...
    path_output = 'Desktop/URECA/GAN-main/outputs'
    ...
    for epoch in range(num_epochs):
        for i, (x1, x2,yy) in enumerate(data_loader):
            ...
            # Total loss
            loss =  mse_loss(gen_images, yy) + 2*bce_loss(gen_images, yy) + 1.2*perceptual_loss(gen_images, yy)
        ...
        if (epoch + 1) % 1 == 0:
            ...
            if loss.item() < min_loss:
                ...
                torch.save({'epoch': epoch+start_ep,
                            ...
                            }, os.path.join(path_model,"checkpoint_neoFYNet_final.pth"))
            #save model
            torch.save({'epoch': epoch+start_ep,
                        'model': model.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        'np_rand_state': np.random.get_state(),
                        'torch_rand_state': torch.get_rng_state(),
                        }, os.path.join(path_model,"checkpoint_neoFYNet.pth"))
    ...
    plt.tight_layout()
    plt.savefig(path_output+f'/eps={num_epochs}_plot_FYNet.png')
    plt.show()
